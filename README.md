...
## Depencencies
You will need ```Gym```, which is required for the machine learning agent, ```Pygame``` which provides the environment for Gym and ```Numpy``` is also required for some array computations and common math formulae.
<!-- CONTRIBUTING -->
## Contributing
